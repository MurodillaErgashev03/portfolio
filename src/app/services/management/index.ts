...
export const managementApi = api.injectEndpoints({
  endpoints: (build) => ({
    ...
    >({
      query: ({ id, params }) => ({
        url: `/management/counterparty/${id}/history${params || ''}`,
      }),
    }),
    ...
    ///////// Contingent Debt Credit /////////
    getContingentDebtCredit: build.mutation<
      IContingentDebtCredit,
      { id: string; params?: string }
    >({
      query: ({ id, params }) => ({
        url: `/management/counterparty/${id}/debt-credit${params || ''}`,
      }),
    }),

    ///////// Contingent Realization /////////
    getContingentRealization: build.mutation<IContingentsRes, string>({
      query: (params) => ({
        url: `/management/counterparty-statistics${params || ''}`,
      }),
    }),
...
export const {
  ...
  //Contingent Debt Credit
  useGetContingentDebtCreditMutation,
  //Contingent Realization
  useGetContingentRealizationMutation,
  ///// Employee /////////
